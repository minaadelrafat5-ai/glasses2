...
### Users & access
- `profiles` — one row per auth user. Holds first/last name, role
  (customer/admin/staff), avatar, and timestamps. `id` matches
  `auth.users.id`. Role lives in a NOT NULL column with a CHECK constraint
  and defaults to 'customer'.
- `user_addresses` — shipping/billing addresses belonging to a user.

### Catalog taxonomy (reference data)
- `brands` — eyewear brands (e.g. Vuera Studio, North Optics).
- `categories` — product categories (optical, sunglasses, blue-light,
  readers) with optional parent for sub-categories.
- `colors` — frame colors (tortoise, matte black, crystal, etc.).
- `frame_shapes` — round, square, oval, cat-eye, aviator, geometric.
- `materials` — acetate, metal, titanium, mixed.
- `sizes` — lens-width sizes in mm (48, 50, 52, 54, 58, 60).

### Catalog
- `products` — the main product row. Slug, name, brand, description,
  shape, material, gender, lens type, base price, compare-at price,
  status, rating, review count, timestamps.
- `product_images` — images for a product, ordered by `position`, with
  an `is_ai_generated` flag for future AI imagery.
- `product_variants` — purchasable variant of a product (color + size +
  lens tint), carrying its own price, stock, and SKU.
- `product_categories` — many-to-many between products and categories.

### Shopping system
- `cart_items` — persisted cart line for a signed-in user (variant +
  quantity). Guests use localStorage; this table syncs on sign-in.
- `wishlist_items` — saved product for a signed-in user.
- `orders` — order header: status, money fields, shipping address
  snapshot, currency. `user_id` references the owning customer.
- `order_items` — line items for an order, snapshotting the product
  name, variant name, unit price, and quantity at purchase time.

### Community
- `reviews` — product reviews with rating, title, body, verified-purchase
  flag, and moderation status.

### Preferences & AI preparation
- `customer_preferences` — free-text + structured preferences
  (style, budget, face profile) for future AI recommendation matching.
- `ai_tryon_requests` — log of AR / AI try-on requests (placeholder for
  future pipeline; no checkout or AI execution yet).
- `ai_recommendation_requests` — log of recommendation prompts
  (placeholder for future pipeline).

## Security (RLS)
- RLS enabled on EVERY table.
- Profiles: a user can SELECT/UPDATE only their own profile. INSERT is
  allowed for a user inserting their own profile. Admins/staff can read
  all profiles.
- Catalog tables (brands, categories, colors, frame_shapes, materials,
  sizes, products, product_images, product_variants, product_categories,
  reviews): public read for anon+authenticated; write restricted to
  admin/staff via a helper function `is_staff()`.
- Cart & wishlist: owner-scoped CRUD for authenticated users.
- Orders & order_items: customer can read their own; admin/staff can read
  all; writes restricted to admin/staff (checkout is not implemented yet,
  so no public INSERT policy on orders).
- Customer preferences: owner-scoped CRUD.
- AI request tables: owner-scoped SELECT/INSERT; admin/staff can read all.

## Important notes
1. `profiles.role` uses a CHECK constraint — only 'customer', 'admin',
   'staff' are valid values. Default is 'customer'.
2. Owner columns (`user_id`) default to `auth.uid()` so client inserts
   that omit the owner still satisfy RLS.
3. All money columns are integer cents to avoid floating-point issues.
   The frontend `formatMoney` helper divides by 100.
4. Slugs are UNIQUE and enforced with a partial index on active products.
5. Stock is tracked per variant, not per product.
6. No checkout or AI execution is implemented — the order and AI tables
   are structural placeholders with appropriate RLS so future work only
   needs to add edge functions and policies.
7. A `is_staff()` SECURITY DEFINER helper checks whether the current
   user's profile role is 'admin' or 'staff'. It is the single source of
   truth for staff-level authorization.
*/

-- ============================================================
-- Helper: is_staff()
-- Returns true when the authenticated user's profile role is admin
-- or staff. SECURITY DEFINER so it can read profiles regardless of
-- the caller's RLS.
-- ============================================================
...
CREATE POLICY "read_brands" ON public.brands
  FOR SELECT TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "staff_write_brands" ON public.brands;
CREATE POLICY "staff_write_brands" ON public.brands
...
CREATE POLICY "read_colors" ON public.colors
  FOR SELECT TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "staff_write_colors" ON public.colors;
CREATE POLICY "staff_write_colors" ON public.colors
  FOR ALL TO authenticated
  USING (public.is_staff())
  WITH CHECK (public.is_staff());
...
CREATE POLICY "read_frame_shapes" ON public.frame_shapes
  FOR SELECT TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "staff_write_frame_shapes" ON public.frame_shapes;
CREATE POLICY "staff_write_frame_shapes" ON public.frame_shapes
  FOR ALL TO authenticated
  USING (public.is_staff())
  WITH CHECK (public.is_staff());
...
CREATE POLICY "staff_write_materials" ON public.materials
  FOR ALL TO authenticated
  USING (public.is_staff())
  WITH CHECK (public.is_staff());
...
CREATE TABLE IF NOT EXISTS public.sizes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  size_mm integer NOT NULL UNIQUE CHECK (size_mm > 0 AND size_mm < 100),
  label text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE POLICY "read_sizes" ON public.sizes
  FOR SELECT TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "staff_write_sizes" ON public.sizes;
CREATE POLICY "staff_write_sizes" ON public.sizes
  FOR ALL TO authenticated
  USING (public.is_staff())
  WITH CHECK (public.is_staff());

-- ============================================================
-- Catalog: products, images, variants, categories join
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.products (
  ...
  gender text NOT NULL DEFAULT 'unisex'
    CHECK (gender IN ('unisex', 'men', 'women')),
  lens_type text NOT NULL DEFAULT 'single-vision'
    CHECK (lens_type IN ('single-vision', 'progressive', 'reading', 'non-prescription', 'sunglasses')),
...
CREATE POLICY "staff_write_product_images" ON public.product_images
  FOR ALL TO authenticated
  USING (public.is_staff())
  WITH CHECK (public.is_staff());
...
CREATE POLICY "read_product_variants" ON public.product_variants
  FOR SELECT TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "staff_write_product_variants" ON public.product_variants;
CREATE POLICY "staff_write_product_variants" ON public.product_variants
  FOR ALL TO authenticated
  USING (public.is_staff())
  WITH CHECK (public.is_staff());
...
CREATE POLICY "read_product_categories" ON public.product_categories
  FOR SELECT TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "staff_write_product_categories" ON public.product_categories;
CREATE POLICY "staff_write_product_categories" ON public.product_categories
  FOR ALL TO authenticated
  USING (public.is_staff())
  WITH CHECK (public.is_staff());
...
CREATE POLICY "select_own_cart" ON public.cart_items
...
CREATE POLICY "update_own_cart" ON public.cart_items
  FOR UPDATE TO authenticated
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "delete_own_cart" ON public.cart_items;
CREATE POLICY "delete_own_cart" ON public.cart_items
...
CREATE POLICY "select_own_wishlist" ON public.wishlist_items
  FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "insert_own_wishlist" ON public.wishlist_items;
CREATE POLICY "insert_own_wishlist" ON public.wishlist_items
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "delete_own_wishlist" ON public.wishlist_items;
CREATE POLICY "delete_own_wishlist" ON public.wishlist_items
  FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
-- No public INSERT/UPDATE/DELETE on orders: checkout is not implemented.
-- Admins/staff manage orders via the service role or future edge functions.
...
CREATE POLICY "read_approved_reviews" ON public.reviews
  FOR SELECT TO anon, authenticated
  USING (moderation_status = 'approved' OR auth.uid() = user_id OR public.is_staff());
...
-- ============================================================
-- Customer preferences (future AI recommendations)
-- ============================================================
...
CREATE POLICY "select_own_preferences" ON public.customer_preferences
  FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "insert_own_preferences" ON public.customer_preferences;
CREATE POLICY "insert_own_preferences" ON public.customer_preferences
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "update_own_preferences" ON public.customer_preferences;
CREATE POLICY "update_own_preferences" ON public.customer_preferences
  FOR UPDATE TO authenticated
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "delete_own_preferences" ON public.customer_preferences;
CREATE POLICY "delete_own_preferences" ON public.customer_preferences
  FOR DELETE TO authenticated USING (auth.uid() = user_id);

-- ============================================================
-- AI feature placeholders (AR try-on, AI images, recommendations)
-- ============================================================
...
CREATE POLICY "select_own_tryon" ON public.ai_tryon_requests
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.is_staff());

DROP POLICY IF EXISTS "insert_own_tryon" ON public.ai_tryon_requests;
CREATE POLICY "insert_own_tryon" ON public.ai_tryon_requests
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "select_own_recommendations" ON public.ai_recommendation_requests
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.is_staff());

DROP POLICY IF EXISTS "insert_own_recommendations" ON public.ai_recommendation_requests;
CREATE POLICY "insert_own_recommendations" ON public.ai_recommendation_requests
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
...
CREATE TRIGGER trg_ai_tryon_updated_at
  BEFORE UPDATE ON public.ai_tryon_requests
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();
